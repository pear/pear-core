...
$config->set('php_dir', 'whoo', 'user', '__uri');
...
$config->set('php_dir', 'whoo');
$phpunit->assertIsa('PEAR_Registry', $config->getRegistry(), 'set php_dir user');
$r = &$config->getRegistry();
$phpunit->assertEquals('whoo' . DIRECTORY_SEPARATOR . '.registry', $r->statedir, 'whoo statedir');
$phpunit->assertFalse($config->getRegistry('system'), 'set php_dir system');

$config->set('php_dir', 'whoop', 'system');
$phpunit->assertIsa('PEAR_Registry', $config->getRegistry(), 'set php_dir user');
$r1 = &$config->getRegistry('user');
$phpunit->assertEquals('whoo' . DIRECTORY_SEPARATOR . '.registry', $r1->statedir, 'whoo statedir 2');
$phpunit->assertIsa('PEAR_Registry', $config->getRegistry('system'), 'set php_dir system');
$r2 = &$config->getRegistry('system');
$phpunit->assertEquals('whoop' . DIRECTORY_SEPARATOR . '.registry', $r2->statedir, 'whoop statedir');
echo 'tests done';
?>
--EXPECT--
tests done
